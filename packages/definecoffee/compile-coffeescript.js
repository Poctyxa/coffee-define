...
//----------------------------------Create define------------------------------
function defineValues(coffee) {
  var defineRegEx, defineArrayItems, execItemArray, defineReplace;
  defineRegEx = new RegExp('[\\s\\t]*#define\\s(\\w+)\\s(.+)\\n', 'gim');
  defineArrayItems = 0;

  var defineArray = new Array();
	while ((execItemArray = defineRegEx.exec(coffee)) !== null) {
    defineArray[defineArrayItems] = {};
    defineArray[defineArrayItems].itemName = execItemArray[1];
    defineArray[defineArrayItems].itemValue = execItemArray[2];

    defineArray[defineArrayItems].define = execItemArray[0];
    defineArrayItems++;
  }

  for (defineReplace = 0; defineReplace < defineArray.length; defineReplace++) {
    coffee = coffee.replace(defineArray[defineReplace].define, "");
    var reg = new RegExp(defineArray[defineReplace].itemName, "gim");
    coffee = coffee.replace(reg, defineArray[defineReplace].itemValue);
  }
  return coffee;
};
